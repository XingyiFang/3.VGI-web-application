<?php
	...
	$image_quality = $_REQUEST["image_quality"]; 
	...
	SET mod_polygon = '$mod_poly', report_id = '$report_id_new', mod_image='$imagename', mod_lat=$lat, mod_lng=$lng, position_acc=$pos_accu, qa_image =$image_quality, mod_loc_des='$loc_des', mod_obs_type = '$types', mod_duration='$duration', mod_urgency='$priority', status='$status', mod_obs_impact='$obs_impact', mod_flag='false', mod_comment='$mod_comment', mod_scale='$mod_scale', mod_obs_des='$obs_des'
	WHERE report_id = '$report_id';";

	if($extend_life != null or $extend_life!=""){
		echo $extend_life;
		$query .= "update records_vgi set close_time = '".$extend_life."' where report_id = '$report_id'";
	}else{
		echo $extend_life;
	};
	
	//$query .= "update records_vgi set close_time = close_time + cast('".$extend_life."' as interval) where report_id = '$report_id'";
	pg_query($db, $query);
	//echo $types
?>