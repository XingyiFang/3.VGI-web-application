<?php
	...
	$query = "SELECT * from obstacles WHERE obs_group = '$obs_id' or obs_group='$obs_id1'";
	...
	while ($row = pg_fetch_row($result)) {
		...
		if(strpos($row[15],'(mod)') !== false){
			...
			<tr><td><strong>image quality</strong></td><td><select id="image_quality"><option>high</option><option>medium</option><option>low</option><option>missing</option></select></td></tr>
			...
			<tr><td><strong>close date</strong></td><td><address>'.$row[45].' || extend:<select id="extend_life"><option>0 day</option><option>1 day</option><option>2 days</option><option>3 days</option><option>4 days</option><option>5 days</option></select></address></td></tr>
			<tr style="color: blue"><td><strong>QA: temporal consistency</strong></td><td id=\'QA_consis\'><address>'.$row[38].'</address></td></tr>
			<tr style="color: blue"><td><strong>QA: location (text)</strong></td><td id=\'QA_loc_text\'><address>'.$row[40].'</address></td></tr>
			<tr style="color: blue"><td><strong>QA: location (X,Y)</strong></td><td id=\'QA_loc_xy\'><address>'.$row[39].'</address></td></tr>
			<tr style="color: blue"><td><strong>QA: duration</strong></td><td id=\'QA_duration\'><address>'.$row[42].'</address></td></tr>
